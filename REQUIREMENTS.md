...
| F9  | Clear user-facing errors for: malformed URL, non-Wikipedia URL, non-English wiki, namespace URL (Special/Talk/etc.), 404 from Wikipedia, disambiguation page, empty or very short article (< 500 chars body), article exceeding length cap (truncated with UI flag per U13), Ollama unreachable, Ollama timeout, **malformed or empty LLM response** (e.g., LLM returns empty string, non-prose, or fails JSON contract where one is expected), vector DB unreachable, retrieval returning zero chunks, chat called before any article is ingested (per A12). Each error returns a structured `{error: <code>, message: <user-facing string>}` payload; the frontend renders the message verbatim. Error codes are enumerated in DESIGN §5. |
